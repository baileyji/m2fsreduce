pro findtrace,str,tracestr,npoly=npoly,pl=pl,stp=stp,sigkern=sigkern,$
                nthreshsig=nthreshsig,thresh=thresh0,peakcol=peakcol0,dcr=dcr,$
                halfwid=halfwid,npts=npts
...
;  =dcr        The separation limit of peaks between neighboring searched columns
;                to be included as part of the same order.  Default is 2.
;  =halfwid    The half-width of the orders, used for creating flux-weighted
;                y-positions.  Default is 3.
;  =npts       The number of columns to search for peaks.  Default is 20.
...
if n_elements(str) eq 0 then begin
  print,'Syntax - findtrace,str,tracestr,npoly=npoly,pl=pl,stp=stp,sigkern=sigkern,'
  print,'                     nthreshsig=nthreshsig,thresh=thresh,peakcol=peakcol,'
  print,'                     halfwid=halfwid,dcr=dcr,npts=npts'
  return
endif
...
gdpix = where((str.mask and 1) eq 0,ngdpix)
...
if n_elements(npts) eq 0 then npts = 20
;xshift = fltarr(npts)
...
for i=0,npts-1 do begin
  ...
  if n_elements(dcr) eq 0 then dcr = 2
  if abs(off-halfup) eq step then begin

    srcor2,peak,peak*0,peak2,peak2*0,dcr,ind1,ind2,opt=1,/silent
  ...
  ; Compare to last one
    ...
    if ngd gt 0 then begin
      srcor2,lastpeak[gd],lastpeak[gd]*0,peak2,peak2*0,dcr,ind1,ind2,opt=1,/silent
      dum = where(ind1 ne -1,nmatch)
  ...
  lastpeak = lastpeak*0-1  ; initialize all lastpeak to bad
  if nmatch gt 0 then $
...
tracestr = REPLICATE({peaky:0.0,wty:0.0,gaussy:0.0,fwhm:0.0,height:0.0,gcoef:fltarr(3),fitcoef:dblarr(npoly+1),$
                      ...
                      coef:fltarr(npoly+1),firstx:0L,lastx:0L},npeak)

;y = lindgen(ny)
x = lindgen(nx)
if n_elements(halfwid) eq 0 then halfwid = 3L 
...
FOR i=0,npeak-1 do begin
  ...
  firstx = -1  ; initialize first/last X values
  lastx = -1
  ...
  For j=0,npts-1 do begin
    ...
    if cenarr[i,j] ge 0 then begin
      ...
      ; Fill in first and last X values
      if firstx eq -1 then firstx=xlo
      lastx = xhi

    endif

  Endfor ; loop through sub-regions
  ...
  tracestr[i].firstx = firstx
  tracestr[i].lastx = lastx
