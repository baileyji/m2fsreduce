...
@unitchecked
def dumb(phi_0, lam_0, phi_1, lam_1):
    '''Calculates the  distance between point 0 and point 1
        ...
        '''
    return np.sqrt((np.cos(phi_0)*(lam_1-lam_0))**2.0 + (phi_1-phi_0)**2.0)
...
def dist_radec(ra0,dec0,ra1,dec1, method='vincenty', *arg, **kwargs):
    '''Calculates the angular distance between point 0 and point 1
    ...
    Keywords
    --------
    method : 'Vincenty' | 'Haversine' | 'Simple'
       default Vincenty
    ...
    m=method.lower()
    if m=='vincenty':
        return Vincenty(dec0,ra0,dec1,ra1,**kwargs)
    elif m=='haversine':
        return Haversine(dec0,ra0,dec1,ra1,**kwargs)
    elif m=='simple':
        return simple(dec0,ra0,dec1,ra1,**kwargs)
    elif m=='dumb':
        return dumb(dec0,ra0,dec1,ra1,**kwargs)
    else:
        raise ValueError('Method must be vincenty, haversine, or simple')
...
def dist_fitrec(rec1,rec2):
    return dist_radec(rec1['RAJ2000'],rec1['DEJ2000'],
                      rec2['RAJ2000'],rec2['DEJ2000'],unit='deg',
                      method='Haversine')
