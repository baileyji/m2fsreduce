...
;be put manually
  ...
  inst_mode = 'm2fs' ;Spectrometer identification to modeinfo
;  exten=1
  prefix  = 'Desktop/' ;Prefix for intermediate products (e.g. master flat)
...
sbiasfile='Desktop/M2FS Commissioning Run/image/b_superbias_93-126.fits'
fits_read,sbiasfile,bias,bhead
...
flat_file='Desktop/M2FS Commissioning Run/image/b_superflat_84-86.fits'
norm_flat_file='Desktop/M2FS Commissioning Run/image/b_superflat_norm_84-86.fits'
fits_read,flat_file,flat,fhead
...
;          or_column=colrange,filter=200.,power=opower,/PLOT

    str={filename:'r0104.fits', header:fhead,$
         flux:float(flat),err:sqrt(float(flat>1)),$
         mask:long(flat*0)}
...
;Plot the trace results
    getwindow,0,xs=1500,ys=1000,/show
    loadct,0
    display,flat,min=0,max=500
...
;Determine extraction windows above and below order maximum.
  getxwd,flat,orders,def_xwd,def_sxwd,colrange=colrange ;get extraction width

;;Save image format description for all orders
;  save,orders,or_range,ord_err,colrange,def_xwd,def_sxwd $
;      ,file=prefix+'.ord.sav'
;
;;  ordim = 0
...
;                                      enough.
  rflat=flat
  hamflat, flat, fhead, orders, blzcoef, colrange=colrange, FXWD=def_xwd $
         , SF_SMOOTH=30., SP_SMOOTH=150, OSAMPLE=12$, swath_width=200 $
         , MASK=mask, /PLOT, THRESHOLD=100,/NOSCATTER
;  writefits,norm_flat_file,flat,fhead ;Write out normalized flat
;fits_read,norm_flat_file,flat,fhead

;;Add blaze functions to the save file
;  save,orders,or_range,ord_err,colrange,def_xwd,def_sxwd,blzcoef $
;      ,file=prefix+'.ord.sav'
;  mask=0
;  flat=0
  stop
...
;           ,sig=sunc,colrange=colrange,/thar
;;    display,im,/log
...
;    im=im-bias                             ; Bias subtraction
    ...
    imfile=fileroot+'77-83.fits'
;    im=bias*0d & for i=77,83 do im+=readfits(fileroot+$
;        string(i,format='(i04)')+'.fits',header)-bias
;   writefits,imfile,im,header
...
;;
;;    orders[0,*]=orders[0,*]-35
;    mkscatter, im, orders, bg, ybg, colrange=colrange, LAMBDA_SF=100. $
;             , SWATH_WIDTH=600, OSAMPLE=10, MASK=mask, LAMBDA_SP=100. $
;             , GAIN=gain, READN=readn, /SUBTRACT
;    spawn,'m2fs_scatter.py "'+imfile+'" "'+scatterfile+'"'
;    im=readfits(scatterfile)/flat
...
;and hamspec
;
    getxwd, im, orders, xwd, sxwd, colrange=colrange ;get extraction width
;    xwd=replicate(30.,2,nord)

    hamspec, im, head, orders, xwd, sxwd, sp $
           , sig=sig, colrange=colrange, SF_SMOOTH=10., OSAMPLE=12 $
           , swath_width=200, MASK=mask, /PLOT, FILENAME=file_basename(imfile)
;    im=0
